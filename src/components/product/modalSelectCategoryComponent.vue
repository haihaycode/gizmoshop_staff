<template>
    <ModalBox :isOpen="isOpen" :loading="isLoading" :closeModal="closeModal" :closeText="'Đóng'">
        <template #header>
            <h3 class="sm:text-sm md:text-lg font-bold">Chọn danh mục </h3>
        </template>
        <template #body>
            <div class=" text-gray-600 p-2">
                <p><span class="inline-block w-4 h-4 bg-red-500 rounded-full"></span> Danh mục không hoạt động</p>
                <p><span class="inline-block w-4 h-4 bg-blue-500 rounded-full"></span> Danh mục đang hoạt động</p>
            </div>
            <CardGridComponent class="bg-white p-2" :icon="`<i class='bx bx-category'></i>`" :items="data"
...
export default {
    name: 'modalSelectCategoryComponent',
    components: {
        ModalBox,
        CardGridComponent
    },
